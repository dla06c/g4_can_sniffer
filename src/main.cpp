...
function setDashNeedle(id, value, min, max) {
  ...
  const angle = 235 + (485 - 235) * pct;

  // Short outer-ring pointer:
  // starts just outside the inner rings and reaches the number/tick band.
  // This prevents the needle from crossing the black inner dial.
  const inner = dashPolar(DASH_CX, DASH_CY, 125, angle);
  const outer = dashPolar(DASH_CX, DASH_CY, 204, angle);
...
function setLowerArcNeedle(id, value, min, max) {
  ...
  const pct = clamp((value - min) / (max - min), 0, 1);
  const angle = 202 + (158 - 202) * pct;

  const inner = dashPolar(DASH_CX, DASH_CY, 126, angle);
  const outer = dashPolar(DASH_CX, DASH_CY, 208, angle);
...
function setProgressArc(id, value, min, max) {
  ...
  const startAngle = 235;
  const endAngle = 235 + (485 - 235) * pct;
  arc.setAttribute("d", dashDescribeArc(DASH_CX, DASH_CY, 225, startAngle, endAngle));
}
