...
function buildCarDashGauges() {
  ...
  initDashNeedle("boostNeedle", 125, 204, 235);
  initDashNeedle("rpmNeedle", 125, 204, 235);
  initDashNeedle("iatNeedle", 126, 208, 202);
  initDashNeedle("ectNeedle", 126, 208, 202);
  initProgressArc("boostProgressArc", 235, 485);
  initProgressArc("rpmProgressArc", 235, 485);
}
...
function setDashNeedle(id, value, min, max) {
  ...
  const baseAngle = Number(needle.dataset.baseAngle || 235);
...
function setLowerArcNeedle(id, value, min, max) {
  ...
  const baseAngle = Number(needle.dataset.baseAngle || 202);
  const rotation = (angle - baseAngle).toFixed(2);
  if (needle.dataset.rotation === rotation) return;
  needle.setAttribute("transform", "rotate(" + rotation + " " + DASH_CX + " " + DASH_CY + ")");
  needle.dataset.rotation = rotation;
