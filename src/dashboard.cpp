...
/*
 * Linebeam.ttf is expected beside index.html/styles.css.
 * It is intentionally referenced rather than bundled.
 */
...
/* ---- Gear (debug) and Light (lighting) scrollable content pages ---------- */
...
.content-card button, .content-card select {
  width: 100%; box-sizing: border-box;
  border: 0; border-radius: 7px;
  padding: 9px 8px;
  background: #1e1e1e; color: #dde2ea;
  font-size: 13px; font-weight: bold; font-family: inherit;
  cursor: pointer; margin-top: 4px;
}
.content-card button.active-btn { background: #004da3; color: #fff; }
.content-card select {
  border: 1px solid #2a2a2a; background: #111; color: #dde2ea;
}
.content-card input[type="color"],
.content-card input[type="range"],
.content-card input[type="number"],
.content-card input[type="text"] {
  width: 100%; box-sizing: border-box;
  border: 1px solid #2a2a2a; border-radius: 6px;
  padding: 7px; background: #111; color: #dde2ea;
...
.main-alert-bar {
  ...
  background: rgba(107,21,21,0.92);
  color: #ffdddd;
...
<div class="stream-log" id="stream-log"></div><div>OIL 74 PSI</div><div>CAN HEARTBEAT OK</div><div>VOLTAGE 14.1V</div><div>WATER 99C / AIR 14C</div><div>RPM 460 / MAP 1.2 PSI</div>
</div>
</section>
...
function updateDashboard({
  ...
  setBar("boost-bar", ((boost + 20) / 270) * 100);
  setBar("oil-bar", (oilPressure / 700) * 100);
...
async function refreshData() {
  ...
  do {
    ...
    try {
      ...
      const NOW = Date.now();
      if (!refreshData._lastLog || NOW - refreshData._lastLog > 3000) {
        refreshData._lastLog = NOW;
        const msgs = [
...
async function applyLighting() {
  ...
  await fetch('/setLighting?mode='+encodeURIComponent(mode)+'&pattern='+encodeURIComponent(pattern)
    +'&r='+color.r+'&g='+color.g+'&b='+color.b+'&w=0&brightness='+brt
    +'&auto_off_minutes='+autoOff+zonePart+pp);
}
