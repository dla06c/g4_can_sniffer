...
.metric-unit {
  font-family: "Linebeam", "Courier New", monospace !important;
...
body {
  margin: 0;
  background:
    radial-gradient(circle at center, rgba(255, 34, 13, 0.08), transparent 46%),
    #090909;
  color: white;
...
.dashboard {
  ...
  aspect-ratio: 226 / 132;
  overflow: hidden;
  background: var(--background);
  box-shadow: 0 0 14px rgba(255, 34, 13, 0.24);
}
...
/* Static background: cached by the browser as a single image resource. */
.dashboard-bg {
  display: block;
  object-fit: fill;
}

/* Dynamic RPM segments — isolated from the static background. */
.rpm-layer {
  display: block;
  overflow: visible;
  pointer-events: none;
...
/*
 * .dashboard-outline, .dashboard-object, #sun, #brand-logo, #car, etc.
 * These selectors are no longer used in this document. The static artwork
 * that bore these classes/IDs has been moved to dashboard-bg.svg where
 * the equivalent rules live inside the SVG's own <style> block.
 */
...
.bar-fill {
  ...
  box-shadow: 0 0 6px rgba(0, 224, 0, 0.28);
...
.top-brand-label {
  ...
  text-shadow: 0 0 8px rgba(255, 34, 13, 0.28);
}
...
.top-tab {
  ...
  place-items: center;
  color: var(--grey);
  border: 1.5px solid rgba(169, 173, 179, 0.9);
  background: #000;
  font-size: clamp(14px, 1.9vw, 34px);
  line-height: 1;
  letter-spacing: 0.04em;
  text-shadow: 0 0 6px rgba(169, 173, 179, 0.18);
}
...
.stream-panel .panel-heading {
  flex: 0 0 auto;
}

.stream-panel .stream-log {
  flex: 1 1 auto;
  min-height: 0;
  padding: 0;
  background: transparent;
...
.connection-status.connected {
  color: #86fa88;
  text-shadow:
    0 0 4px rgba(134, 250, 136, 0.50),
    0 0 9px rgba(134, 250, 136, 0.22);
}

.connection-status.disconnected {
  color: #d33f6a;
  text-shadow:
    0 0 4px rgba(211, 63, 106, 0.50),
    0 0 9px rgba(211, 63, 106, 0.22);
}

/* -------------------------------------------------------------------------
   RPM number follows the colour of the highest active RPM bar segment.
   The actual colour is assigned by app.js.
   ------------------------------------------------------------------------- */
.rpm-readout {
...
/* On blank dummy pages, hide all drive-specific artwork layers. */
.dashboard[data-active-page="gear"] .dashboard-bg,
.dashboard[data-active-page="gear"] .rpm-layer,
.dashboard[data-active-page="gear"] .gear-indicator-layer,
.dashboard[data-active-page="light"] .dashboard-bg,
.dashboard[data-active-page="light"] .rpm-layer,
.dashboard[data-active-page="light"] .gear-indicator-layer {
  display: none;
}
...
.dashboard-tab:focus-visible {
  color: #b4b4b4;
  outline: none;
  filter: drop-shadow(0 0 4px rgba(180, 180, 180, 0.24));
}

.dashboard-tab.is-active {
  color: var(--red);
  filter:
    drop-shadow(0 0 2px rgba(255, 34, 13, 0.58))
    drop-shadow(0 0 7px rgba(255, 34, 13, 0.22));
}
...
.dashboard-tab.tab-1 { left: 37.0%; }
.dashboard-tab.tab-2 { left: 46.8%; }
.dashboard-tab.tab-3 { left: 56.6%; }
...
@media (max-width: 640px) {
  ...
  .dashboard-tab.tab-1 { left: 35.6%; }
  .dashboard-tab.tab-2 { left: 46.0%; }
  .dashboard-tab.tab-3 { left: 56.4%; }
}


/* -------------------------------------------------------------------------
   Current-gear indicator
   Active colour is driven by CSS [data-gear-color] attribute selectors —
...
.gear-progress-segment {
  ...
  filter: none;
  vector-effect: non-scaling-stroke;
  /* `filter` is intentionally absent from the transition list: animating
     SVG filter effects forces a re-rasterise of the composited layer on
     every frame, which is expensive. The drop-shadow switches instantly
     with the is-active class instead. */
  transition:
    stroke 90ms linear,
    stroke-width 90ms linear,
    stroke-opacity 90ms linear,
    fill 90ms linear,
    fill-opacity 90ms linear;
}
...
.gear-progress-segment.is-active {
  stroke-width: 2.7px;
  stroke-opacity: 1;
  fill-opacity: 0.06;
}
...
/* Green (gear 1) */
.gear-progress-segment.is-active[data-gear-color="#86fa88"] {
  stroke: #86fa88;
  fill: #86fa88;
  filter: drop-shadow(0 0 2px #86fa88) drop-shadow(0 0 6px #86fa88);
}
.gear-progress-label.is-active[data-gear-color="#86fa88"] {
  fill: #86fa88;
  filter: drop-shadow(0 0 2px #86fa88) drop-shadow(0 0 5px #86fa88);
}

/* Yellow (gear 2) */
.gear-progress-segment.is-active[data-gear-color="#d0d000"] {
  stroke: #d0d000;
  fill: #d0d000;
  filter: drop-shadow(0 0 2px #d0d000) drop-shadow(0 0 6px #d0d000);
}
.gear-progress-label.is-active[data-gear-color="#d0d000"] {
  fill: #d0d000;
  filter: drop-shadow(0 0 2px #d0d000) drop-shadow(0 0 5px #d0d000);
}

/* Orange (gear 3) */
.gear-progress-segment.is-active[data-gear-color="#ff8000"] {
  stroke: #ff8000;
  fill: #ff8000;
  filter: drop-shadow(0 0 2px #ff8000) drop-shadow(0 0 6px #ff8000);
}
.gear-progress-label.is-active[data-gear-color="#ff8000"] {
  fill: #ff8000;
  filter: drop-shadow(0 0 2px #ff8000) drop-shadow(0 0 5px #ff8000);
}

/* Red (gears 4 & 5) */
.gear-progress-segment.is-active[data-gear-color="#d33f6a"] {
  stroke: #d33f6a;
  fill: #d33f6a;
  filter: drop-shadow(0 0 2px #d33f6a) drop-shadow(0 0 6px #d33f6a);
}
.gear-progress-label.is-active[data-gear-color="#d33f6a"] {
  fill: #d33f6a;
  filter: drop-shadow(0 0 2px #d33f6a) drop-shadow(0 0 5px #d33f6a);
}


/* -------------------------------------------------------------------------
   Gear indicator paint isolation

   The animated gear indicator is intentionally rendered in its own SVG layer.
   Changing gear no longer invalidates/repaints the complete wireframe SVG.
   ------------------------------------------------------------------------- */
...
/* -------------------------------------------------------------------------
   Animated road / tire-tread motion layer
   ------------------------------------------------------------------------- */
.scene-motion-layer {
  position: absolute;
  left: 0;
...
.dashboard[data-active-page="gear"] .scene-motion-layer,
.dashboard[data-active-page="light"] .scene-motion-layer {
  display: none;
}
...
.far-road-line {
  ...
  stroke-opacity: 1;
  filter:
    drop-shadow(0 0 3px rgba(208, 208, 0, 0.65))
    drop-shadow(0 0 6px rgba(208, 208, 0, 0.22));
}
...
.road-motion-line.is-lit {
  stroke: #d0d000;
  stroke-width: 1px;
  fill: rgba(255, 231, 14, 0.212) !important;
  stroke-opacity: 1;
  filter:
...
.tire-tread-line.is-lit {
  stroke: #ff220d;
  stroke-width: 1px;
  stroke-opacity: 1;
  filter:
    drop-shadow(0 0 3px rgba(255, 34, 13, 0.78))
    drop-shadow(0 0 6px rgba(255, 34, 13, 0.26));
}
...
<stop offset="18.749%" stop-color="#86fa88"></stop>
...
<div class="gear-indicator-layer" aria-hidden="true">
  <svg
    class="gear-indicator-svg"
    viewBox="196.999969 -211.000000 226.000031 132.000000"
    preserveAspectRatio="xMidYMid meet"
  >
    <g transform="matrix(1,0,0,-1,0,0)">
      <g id="gear-indicator" aria-label="Current gear indicator">
    <g class="gear-indicator-item" data-gear-item="1">
      <path
        id="gear-indicator-segment-1"
        class="vector-outline gear-progress-segment"
        data-gear="1"
        data-gear-color="#86fa88"
        d="M207.078857,155.404099L198.078857,146.404099L198.078857,155.404099L207.078857,155.404099"
      ></path>
      <text
        id="gear-indicator-label-1"
        class="gear-progress-label"
        data-gear-label="1"
        data-gear-color="#86fa88"
        x="201.2"
        y="-151.55"
    ...
    <g class="gear-indicator-item" data-gear-item="2">
      <path
        id="gear-indicator-segment-2"
        class="vector-outline gear-progress-segment"
        data-gear="2"
        data-gear-color="#d0d000"
        d="M212.078857,160.404099L198.078857,160.404099L198.078857,156.404099L208.078857,156.404099L212.078857,160.404099"
      ></path>
      <text
        id="gear-indicator-label-2"
        class="gear-progress-label"
        data-gear-label="2"
        data-gear-color="#d0d000"
        x="204.1"
    ...
    <g class="gear-indicator-item" data-gear-item="3">
      <path
        id="gear-indicator-segment-3"
        class="vector-outline gear-progress-segment"
        data-gear="3"
        data-gear-color="#ff8000"
        d="M217.078857,165.404099L213.078857,161.404099L198.078857,161.404099L198.078857,165.404099L217.078857,165.404099"
      ></path>
      <text
        id="gear-indicator-label-3"
        class="gear-progress-label"
        data-gear-label="3"
        data-gear-color="#ff8000"
        x="206.1"
    ...
    <g class="gear-indicator-item" data-gear-item="4">
      <path
        id="gear-indicator-segment-4"
        class="vector-outline gear-progress-segment"
        data-gear="4"
        data-gear-color="#d33f6a"
        d="M222.078857,170.404099L198.078857,170.404099L198.078857,166.404099L218.078857,166.404099L222.078857,170.404099"
      ></path>
      <text
        id="gear-indicator-label-4"
        class="gear-progress-label"
        data-gear-label="4"
        data-gear-color="#d33f6a"
        x="208.85"
    ...
    <g class="gear-indicator-item" data-gear-item="5">
      <path
        id="gear-indicator-segment-5"
        class="vector-outline gear-progress-segment"
        data-gear="5"
        data-gear-color="#d33f6a"
        d="M227.078842,175.404099L223.078842,171.404099L198.078857,171.404099L198.078857,175.404099L227.078842,175.404099"
      ></path>
      <text
        id="gear-indicator-label-5"
        class="gear-progress-label"
        data-gear-label="5"
        data-gear-color="#d33f6a"
        x="211.3"
...
<div class="scene-motion-layer" aria-hidden="true">
  <svg
    class="scene-motion-svg"
    viewBox="197.285278 -52.404114 225.500732 32.000015"
    preserveAspectRatio="none"
  >
    <g id="scene-motion-far-road">
      <path class="scene-motion-path far-road-line is-lit" data-source-index="0" transform="matrix(1,0,0,-1,0,-20)" d="M290.346893,30L290.820679,30L294.978943,32.404114L290.346893,30"></path>
      <path class="scene-motion-path far-road-line is-lit" data-source-index="1" transform="matrix(1,0,0,-1,0,-20)" d="M329.120605,30L329.593201,30L324.961182,32.404114L329.120605,30"></path>
...
<div aria-hidden="false" class="overlay-layer dashboard-page drive-page is-active" data-page="drive">



<section aria-label="Stream status log" class="stream-panel">
...
const gearSegments = Array.from(
  document.querySelectorAll(".gear-progress-segment[data-gear]")
).sort(
  (a, b) => Number(a.dataset.gear) - Number(b.dataset.gear)
);

const gearLabels = Array.from(
  document.querySelectorAll(".gear-progress-label[data-gear-label]")
);
...
function registerPacketReceived(timestamp = performance.now()) {
  lastPacketReceivedAt = timestamp;
  setConnectionState(true);
}

function checkConnectionState(timestamp = performance.now()) {
  const connected =
    lastPacketReceivedAt > 0 &&
    timestamp - lastPacketReceivedAt <= PACKET_TIMEOUT_MS;
...
function setRpmReadoutColor(color) {
  if (!rpmReadout || color === lastRpmColor) return;
  lastRpmColor = color;

  rpmReadout.style.color = color;

  if (color === "#272727") {
    rpmReadout.style.textShadow = "0 0 6px rgba(80, 80, 80, 0.18)";
    return;
  }

  rpmReadout.style.textShadow =
...
function renderSceneMotionFrame() {
  const nextRoadIndexes = spacedMotionIndexes(
    roadMotionFrame,
    roadMotionGroups.length,
    ROAD_VISIBLE_GROUPS
  );
  const nextTreadIndexes = spacedMotionIndexes(
    tireMotionFrame,
    tireTreadGroups.length,
    TREAD_VISIBLE_GROUPS
  );
...
let previousGearIndicator = null;

function setGearIndicator(gear) {
  const numericGear = Number.parseInt(gear, 10);
  const selectedGear =
    Number.isInteger(numericGear) && numericGear >= 1 && numericGear <= 5
      ? numericGear
      : 0;

  if (selectedGear === previousGearIndicator) {
    return;
  }

  previousGearIndicator = selectedGear;

  // Toggle `is-active` only — colours are driven entirely by CSS
  // [data-gear-color] attribute selectors; no inline style writes.
  gearSegments.forEach((segment) => {
    segment.classList.toggle(
      "is-active",
      Number(segment.dataset.gear) === selectedGear
    );
  });
  ...
  gearLabels.forEach((label) => {
    label.classList.toggle(
      "is-active",
      Number(label.dataset.gearLabel) === selectedGear
    );
  });
}
...
const setBar = (id, percentage, color) => {
  const element = document.getElementById(id);
  if (!element) return;
  const ratio = clamp(percentage, 0, 100) / 100;
  const formatted = ratio.toFixed(3);
  const nextColor = color || element.style.background || "";
  const state = `${formatted}:${nextColor}`;
  if (_barCache.get(id) === state) return;
  _barCache.set(id, state);
  element.style.transform = `scaleX(${formatted})`;
  if (color) element.style.background = color;
};

function oilBarColor(oilPressure) {
  const value = finiteNumber(oilPressure);
  if (value < 20) return DASHBOARD_COLORS.red;
  if (value < 25) return DASHBOARD_COLORS.orange;
  if (value < 30) return DASHBOARD_COLORS.yellow;
  if (value < 50) return DASHBOARD_COLORS.green;
  if (value < 60) return DASHBOARD_COLORS.yellow;
  if (value < 70) return DASHBOARD_COLORS.orange;
  return DASHBOARD_COLORS.pink;
}

function voltageBarColor(voltage) {
  const value = finiteNumber(voltage);
  if (value < 10) return DASHBOARD_COLORS.red;
  if (value < 11.5) return DASHBOARD_COLORS.pink;
  if (value < 12.5) return DASHBOARD_COLORS.yellow;
  return DASHBOARD_COLORS.green;
}

function waterBarColor(waterTemp) {
  const value = finiteNumber(waterTemp);
  if (value < 80) return DASHBOARD_COLORS.blue;
  if (value < 105) return DASHBOARD_COLORS.green;
  if (value < 110) return DASHBOARD_COLORS.yellow;
  if (value < 115) return DASHBOARD_COLORS.orange;
  return DASHBOARD_COLORS.red;
}

function airBarColor(airTemp) {
  const value = finiteNumber(airTemp);
  if (value < 15) return DASHBOARD_COLORS.blue;
  if (value < 20) return DASHBOARD_COLORS.green;
  if (value < 25) return DASHBOARD_COLORS.yellow;
  if (value < 30) return DASHBOARD_COLORS.orange;
  return DASHBOARD_COLORS.red;
}
...
function setBoostBar(boost) {
  ...
  element.style.background = isPositive ? DASHBOARD_COLORS.pink : DASHBOARD_COLORS.blue;
...
function updateDashboard({
  ...
  setText("voltage-value", Number(voltage).toFixed(1));
  setText("water-value", Math.round(waterTemp).toString());
  setText("air-value", Math.round(airTemp).toString());

  setBoostBar(boost);
  setBar("oil-bar", (oilPressure / 100) * 100, oilBarColor(oilPressure));
  setBar("voltage-bar", (voltage / 15) * 100, voltageBarColor(voltage));
  setBar("water-bar", (waterTemp / 120) * 100, waterBarColor(waterTemp));
  setBar("air-bar", (airTemp / 40) * 100, airBarColor(airTemp));
}
...
window.setGearIndicator = setGearIndicator;
window.pushLog = pushLog;



window.registerPacketReceived = registerPacketReceived;
window.checkConnectionState = checkConnectionState;
...
const dashboardTabs = Array.from(
  document.querySelectorAll(".dashboard-tab[data-page-target]")
);
const dashboardPages = Array.from(
  document.querySelectorAll(".dashboard-page[data-page]")
);

function selectDashboardPage(pageName) {
  const pageExists = dashboardPages.some(
    (page) => page.dataset.page === pageName
  );
  ...
  dashboardTabs.forEach((tab) => {
    const selected = tab.dataset.pageTarget === pageName;
...
dashboardTabs.forEach((tab) => {
  tab.addEventListener("click", () => {
    selectDashboardPage(tab.dataset.pageTarget);
  });
});

selectDashboardPage("drive");
window.selectDashboardPage = selectDashboardPage;


window.setSceneMotionActive = setSceneMotionActive;
